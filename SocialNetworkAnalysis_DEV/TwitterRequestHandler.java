...
import SocialNetworkAnalysis.BadIDException;
import SocialNetworkAnalysis.BadUserException;
import SocialNetworkAnalysis.Post;
import SocialNetworkAnalysis.RedundantEntryException;
import SocialNetworkAnalysis.Repost;
	...
	static void getReposted(Post p) throws NumberFormatException, TwitterException, BadUserException {
		...
		while (cursor != 0) {
			...
			for (long id : pages.getIDs()) {
				...
				User reposter = Post.getSample().getUsers().get(Long.toString(id));
